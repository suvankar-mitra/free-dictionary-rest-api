...
public interface WordEntryMapper {
    public static WordEntryDTO entityToDto(final String word, final String langCode,
        ...
        for (WordEntryEntity entity : entities) {
            ...
            if (entity.getSenses() != null) {
                ...
                for (SenseEntity senseEntity : entity.getSenses()) {
                    ...
                    senseDTO
                            .setRelated(new HashSet<>(RelatedMapper.entityToRelatedList(senseEntity.getRelated())));
                    senseDTO.setSynonyms(
                            new HashSet<>(SynonymMapper.senseEntityToStringList(senseEntity.getSenseSynonyms())));
                    senseDTO.setAntonyms(
                            new HashSet<>(AntonymMapper.senseEntityToStringList(senseEntity.getSenseAntonyms())));
                    senseDTO
                            .setExamples(new HashSet<>(ExampleMapper.entityToStringList(senseEntity.getExamples())));
        ...
        String audioUrl = entities.stream().filter(entity -> entity.getSounds() != null)
                .flatMap(entity -> entity.getSounds().stream())
                .filter(sound -> sound != null && sound.getMp3Url() != null && !sound.getMp3Url().isEmpty())
                .map(sound -> sound.getMp3Url())
                .findFirst()
                .orElseGet(() -> entities.stream().filter(entity -> entity.getSounds() != null)
                        .flatMap(entity -> entity.getSounds().stream())
                        .filter(sound -> sound != null && sound.getOggUrl() != null && !sound.getOggUrl().isEmpty())
